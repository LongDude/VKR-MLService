...
Настройка process-логов централизована в `core.logging`. Формат строк:

```text
timestamp level logger event key=value
```

Уровень задается через `LOG_LEVEL`: `DEBUG`, `INFO`, `WARNING`, `ERROR` или
`CRITICAL`. Значение по умолчанию - `INFO`. Неизвестное значение считается
ошибкой конфигурации.

Для CLI доступны флаги детализации:

```powershell
.\scripts\cli.ps1 tasks openalex-cooldown-status -v
.\scripts\cli.ps1 data init-qdrant 1024 -vv
.\scripts\cli.ps1 ml extract-keywords --help
.\scripts\cli.ps1 worker run -v
```

Без `-v` process-логи используют `LOG_LEVEL`, а HTTP/Qdrant dependency logs
ограничены уровнем `WARNING`. `-v` включает полезные dependency logs уровня
`INFO`, `-vv` включает `DEBUG`.

Process-логи описывают lifecycle API, CLI, worker, фасадов и внешних сервисов.
`MLEvent` остается отдельным механизмом статуса ML-задач: он используется для
консольного прогресса и, при включенном `ML_WORKER_EVENT_REDIS`, для Redis-status.

Просмотр логов контейнеров:

```sh
docker compose logs -f vkr-ml-api vkr-ml-worker
```
