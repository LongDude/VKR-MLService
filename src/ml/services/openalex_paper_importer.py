...
class OpenAlexPaperImporter:
    ...
    def _clean_paper(self, paper: ExternalPaperDTO) -> ExternalPaperDTO:
        ...
        return paper.model_copy(
            update={
                ...
                "external_id": paper.external_id.strip() if paper.external_id else None,
    ...
    def _import_with_fallback(
        ...
        existing_papers = [
            paper for paper in papers if self.paper_keys(paper) & existing_keys
        ]
        ...
        try:
            upload_result = self._uploader(session).import_batch(papers_to_import)
            ...
            if not skip_existing:
                updated_count = min(
                    ...
                    sum(
                        ...
                        if self.paper_keys(paper) & existing_keys
                    ),
                )
    ...
    def _merge_result(
        ...
        target.skipped_empty_abstract += source.skipped_empty_abstract
...
OpenAlexBatchImporter = OpenAlexPaperImporter


__all__ = ["OpenAlexBatchImporter", "OpenAlexPaperImporter"]
